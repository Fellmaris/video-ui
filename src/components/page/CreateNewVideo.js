                    ...
                    <Container disableGutters maxWidth="sm" component="main" sx={{pt: 8, pb: 6}}>
                        <Paper elevation={3} sx={{py: 5, px:5}}>
                            ...
                            <Form className = "video-input-padding-x" autoComplete="off">
                                ...
                                {/*                type="file"*/}
                                {/*                fieldName="videofile"*/}
                                {/*                onChange = {(event)=> {onUploadVideo(event.currentTarget.files[0]);}}*/}
                                {/*                />*/}
                                {/*<TextFieldInput error={props.touched.imagefile && !!props.errors.imagefile}*/}
                                {/*                type="file"*/}
                                {/*                fieldName="imagefile"*/}
                                {/*                onChange = {(event)=> {onUploadImage(event.currentTarget.files[0]);}}*/}
                                {/*/>*/}
                                <input id="file" name="videofile" type="file"  onChange={(event) => {
                                    onUploadVideo(event.currentTarget.files[0]);
                                }}/>
                                <input id="file" name="imagefile" type="file"  onChange={(event) => {
